...
for i in range(N):
    ...
    key = open("net_certs_dev/team%d-net/private/team%d-client.key" % (i, i)).read().strip()

    data = CLIENT_DATA.format(SERVER % i, 30000+i, ca, cert, key)
    open("client_entergame_dev/%d.conf" % i, "w").write(data)

print("Finished, check ./client_entergame_dev dir")
